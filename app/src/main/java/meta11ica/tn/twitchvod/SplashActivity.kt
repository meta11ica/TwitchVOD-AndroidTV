...
class SplashActivity : AppCompatActivity() {
        ...
        lifecycleScope.launch {
            ...
            (0 until JSONArray(strResponse).length()).forEach {
                ...
                try {
                    ...
                    val jsonedges = userJsonObject.getJSONObject("data").getJSONObject("searchFor")
                        .getJSONObject("videos").getJSONArray("edges")
    ...
    for (i in 0 until jsonedges.length()) {
        ...
        title.add(item.getJSONObject("item").getString("title"))

        val desc = if (!item.getJSONObject("item").isNull("game")) {
            item.getJSONObject("item").getJSONObject("game").getString("name")
        } else {
            "Untitled"
        }
        description.add(desc)
        studio.add(("Twitch"))
        id.add(item.getJSONObject("item").getLong("id"))
        videoUrl.add(
            "https://www.twitch.tv/videos/" + item.getJSONObject("item")
                .getString("id")
        )
        duration.add(item.getJSONObject("item").getLong("lengthSeconds"))
        if(item.getJSONObject("item").getString("previewThumbnailURL").contains("_404/404")) {
            bgImageUrl.add(
            ...
            bgImageUrl.add(
                item.getJSONObject("item").getString("previewThumbnailURL").replace("214x120","1920x1080")
            )

            cardImageUrl.add(
                item.getJSONObject("item").getString("previewThumbnailURL")
            )
        }
    }
}
    ...
    fun buildVODStreamsObjectRequest(favStreamers: List<String>):JSONArray {
        ...
        */
        for (channelOwnerLogin in favStreamers) {
            val variablesObject = JSONObject()
            variablesObject.put("videoSort", "TIME")
            variablesObject.put("query", channelOwnerLogin)
